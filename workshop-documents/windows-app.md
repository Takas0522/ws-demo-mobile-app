...
VS CodeやCoding Agentで利用できる形のカスタムエージェントを構成します下記のドキュメントや記事を**正確に**参考し作成してください。
...
``` markdown
https://github.com/github/awesome-copilot/blob/main/instructions/agents.instructions.md を参考に作成されたファイルを改善してください。
```
...
``` markdown
Excelで構築された設計書を参照し、WindowsアプリとWindowsアプリで使用するWebAPIの変更を実施します。
