...
``` markdown
モバイルアプリケーションで利用する Web API を動作させるためのコマンド一式を提示してください。
```
...
``` markdown
Android アプリの `testProductCreation` と iOS アプリの `testProductDecoding` を対象にテストを作成してください。
```
...
``` markdown
Excel で構築された設計書を参照し、モバイルアプリのレイアウト変更を実施します。
モバイルアプリで使用する Web API は別部隊で改修済みです。
