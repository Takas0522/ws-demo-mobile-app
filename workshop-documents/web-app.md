...
```markdown
現在の`admin-struts`の実装とドキュメントを参考にして、e2eテストのシナリオを構築してください。
...
動作している様子を確認したいので headed モードで実施します。
...
まずはテスト的に作成するため認証画面をターゲットにテストを構築します。
動作している様子を確認したいので e2eテストは headed モードで実施します。
テスト実装後にテストの実施を行い、テストが失敗していれば修正を行います。
```
