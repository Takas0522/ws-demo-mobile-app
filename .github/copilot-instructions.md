...
- **Windowsアプリ専用**: C++ API サーバー（CppApiServer）を使用する場合、Java環境不要。ソリューションからF5で両方同時起動可能
...
Windowsアプリは `http://localhost:8081/api/mobile` に接続します。バックエンドは以下の2つの構成から選択可能：

1. **C++ API サーバー** (`src/windows-app/CppApiServer/`) — 軽量、Java不要、SQLite直接アクセス
2. **Java BFF + Web API** (`src/mobile-bff/` + `src/web-api/`) — 従来構成
...
|------|------|
| 言語 | C++20 |
| HTTP サーバー | cpp-httplib (header-only) |
| データベース | SQLite3 (amalgamation, C API) |
| JSON | nlohmann/json |
| JWT 認証 | Windows BCrypt API (HMAC-SHA256) |
| パスワードハッシュ | bcrypt (C 実装) |
| ビルド | MSBuild (.vcxproj) |

### アーキテクチャ

- Java BFF + Web API を1つのC++プロセスに統合
- SQLite に直接アクセス（JDBC不要）
- Windowsアプリと同じソリューション内（F5で同時起動可能）

### ディレクトリ構造

```
src/windows-app/CppApiServer/
├── CppApiServer.vcxproj
├── src/
│   ├── main.cpp              # サーバーエントリポイント
│   ├── Server.h / .cpp       # HTTP サーバー管理
│   ├── Models/               # データモデル
│   ├── Database/             # SQLite アクセス (Repository パターン)
│   ├── Services/             # ビジネスロジック
│   ├── Middleware/           # JWT 認証ミドルウェア
│   ├── Routes/               # API エンドポイント定義
│   └── Utils/                # JWT, UUID, JSON ヘルパー
└── include/                  # サードパーティヘッダー
    ├── httplib.h
    ├── sqlite3.h / .c
    ├── nlohmann/json.hpp
