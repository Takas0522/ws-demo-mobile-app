...
#### Bash（Mac/Linux）
```bash
cat <<'EOF' | dotnet run - -- "<引数>"
<C# コード>
EOF
```

**注意点:**
- Bashでは `<<'EOF'` のようにシングルクォートで囲むことで、ヒアドキュメント内の変数展開を防ぎます
- PowerShellの `@'...'@` とBashの `<<'EOF'...EOF` は同等の機能です
- EOF は任意の識別子に変更可能ですが、慣例として EOF を使用します
...
```bash
cat <<'EOF' | dotnet run - -- "<ファイルパス>" "<出力先ディレクトリ>"
#:package ClosedXML@0.105.0
