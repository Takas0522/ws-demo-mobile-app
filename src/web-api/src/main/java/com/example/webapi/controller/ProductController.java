...
import com.example.webapi.service.ProductPriceHistoryService;
...
public class ProductController {
     ...
     *
     * @param productId 商品ID
     * @param startDate 検索開始日（任意、デフォルト: 5年前）
     * @param endDate   検索終了日（任意、デフォルト: 現在）
     * @param page      ページ番号（デフォルト: 1）
     * @param limit     1ページあたりの件数（デフォルト: 20、最大100）
     * @return 価格履歴レスポンス
     */
    @GetMapping("/{productId}/price-history")
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public ResponseEntity<ApiResponse<Map<String, Object>>> getPriceHistory(
            @PathVariable Long productId,
        ...
        log.info("価格履歴取得リクエスト: productId={}, startDate={}, endDate={}, page={}, limit={}",
        ...
        Page<PriceHistoryResponse> historyPage = priceHistoryService.getPriceHistory(
                productId, start, end, page - 1, effectiveLimit);
        ...
        Map<String, Object> data = new HashMap<>();
        data.put("priceHistory", historyPage.getContent());
        data.put("pagination", pagination);
