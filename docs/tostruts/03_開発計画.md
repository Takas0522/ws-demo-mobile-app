...
> **注意**: 本ドキュメントは計画段階で作成されたものです。実装では外部 Tomcat 7 ではなく `tomcat7-maven-plugin`（組込み Tomcat）を採用しました。  
> Dockerfile への Tomcat インストールおよび `CATALINA_HOME` 関連の記述は現在の実装には該当しません。
